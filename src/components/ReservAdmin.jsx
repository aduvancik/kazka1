        ...
        const fetchReservations = async () => {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    const tables = Array(8).fill('').map((_, index) => data.tables?.[`table_${index}`] || '');
                        ...
                        <div className="table-container">
                            {tableOrder.map((originalIndex, displayIndex) => (
                                <div key={originalIndex} className="table-wrapper">
                                    <div
                                        className={`table ${originalIndex < 3 ? 'table-small' : 'table-large'}`}
                                        onClick={() => handleItemClick('tables', originalIndex)}
                                    >
                                        {originalIndex < 3 ? 'Малий стіл' : 'Великий стіл'} {originalIndex + 1}
                                    </div>
